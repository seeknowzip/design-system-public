...
(() => {
  ...
  const stylesheet = `
    :host {
      ...
      background: #000;
      color: #fff;
      /* Local font inheritance patch: was a bare system stack, which shadowed the page's
       * --font-sans (tokens/base.css sets it on body, but :host wins for
       * slotted slides). Slides then rendered in Apple SD Gothic Neo on macOS
       * and 맑은 고딕/Segoe UI on Windows — different metrics, so line breaks
       * and 1080px overflow were machine-dependent. The fallback keeps the
       * component usable on pages that define no --font-sans. */
      font-family: var(--font-sans, -apple-system, BlinkMacSystemFont, "Helvetica Neue", Helvetica, Arial, sans-serif);
      overflow: hidden;
