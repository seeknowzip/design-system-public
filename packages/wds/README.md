...
This repository is an independent local snapshot. Its `@wanteddev/*` package
names are internal workspace identifiers, not packages to fetch from Wanted's
registry. Use the local workspace or a verified local package set. For Local Design System
consumer setup, see `README.md` at the repository root.
